...
const LANGUAGES = { 
  ...
  en: { code: 'en-US', label: 'EN', placeholder: 'Try: "focus on mmsi 235012345" or "zoom to Bangladesh"' }, 

  bn: { code: 'bn-BD', label: 'বাং', placeholder: 'লিখুন: "mmsi 235012345 এ ফোকাস করো" অথবা "বাংলাদেশে জুম করো"' }, 
...
export default function AICommandBar({ vesselMapRef }) { 
  ...
  function handleStop() { 
    ...
    requestIdRef.current++; // invalidate anything in flight -- UI unblocks immediately 
    ...
    // Best-effort: also ask the model to stop generating tokens so it's 

    // not still burning GPU time in the background. Safe to ignore 

    // errors here -- it can throw if called outside active generation. 

    // getEngine().then((engine) => { 

    //   try { engine.interruptGenerate(); } catch (err) {console.log(err.messages) /* not generating -- fine */ } 
  ...
  function handleMicClick() { 
    ...
    recognition.lang = LANGUAGES[lang].code; // switches with the EN/BN toggle -- see Section 3 
  ...
  for (let i = 0; i < event.results.length; i++) {
    ...
    if (event.results[i].isFinal) isFinal = true;
      ...
      if (isFinal) runCommand(transcript); 
        ...
        <div style={styles.langToggle}> 
          ...
          {Object.entries(LANGUAGES).map(([key, { label }]) => ( 
            ...
            <button 
              ...
              style={{ ...styles.langButton, ...(lang === key ? styles.langButtonActive : {}) }} 
...
const styles = { 
  ...
  bar: { 
    ...
    padding: '10px 14px', display: 'flex', flexDirection: 'column', gap: 6, width: 480, 
  ...
  langToggle: { display: 'flex', border: '1px solid #d1d5db', borderRadius: 6, overflow: 'hidden' }, 

  langButton: { padding: '8px 8px', fontSize: 12, border: 'none', background: '#f9fafb', cursor: 'pointer' }, 
  ...
  micButtonActive: { border: '1px solid #dc2626', background: '#fee2e2', color: '#dc2626' }, 

  button: { padding: '8px 12px', fontSize: 14, border: 'none', borderRadius: 6, background: '#2563eb', color: '#fff', cursor: 'pointer' }, 

  stopButton: { padding: '8px 12px', fontSize: 14, border: 'none', borderRadius: 6, background: '#dc2626', color: '#fff', cursor: 'pointer' }, 
